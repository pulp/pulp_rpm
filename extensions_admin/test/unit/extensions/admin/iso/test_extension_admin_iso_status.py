...
from pulp_rpm.extensions.admin.iso import status
...
class TestISOStatusRenderer(unittest.TestCase):
    ...
    @mock.patch(
        'pulp_rpm.extensions.admin.iso.status.ISOStatusRenderer._display_manifest_sync_report',
        side_effect=status.ISOStatusRenderer._display_manifest_sync_report, autospec=True)
    @mock.patch('pulp_rpm.extensions.admin.iso.status.ISOStatusRenderer._display_iso_sync_report',
    ...
    def test_display_report(self, _display_iso_sync_report, _display_manifest_sync_report,
        ...
        progress_report = {
            ids.TYPE_ID_IMPORTER_ISO: {u'traceback': None, u'error_message': None,
                                       ...
                                       u'state_times': {u'not_started': u'2013-04-30T20:37:25',
                                                        u'manifest_in_progress':
                                                        u'2013-04-30T20:37:25'},
        ...
        # All three output methods should have been called with the appropriately instantiates
        # reports objects
        self.assertEqual(_display_iso_sync_report.call_count, 1)
        self.assertEqual(type(_display_iso_sync_report.mock_calls[0][1][1]),
        ...
        self.assertEqual(type(_display_publish_report.mock_calls[0][1][1]),
                         progress.PublishProgressReport)
    ...
    @mock.patch('pulp_rpm.extensions.admin.iso.status.ISOStatusRenderer._display_iso_sync_report',
                side_effect=status.ISOStatusRenderer._display_iso_sync_report, autospec=True)
    def test_display_report_cancelled_sync(self, _display_iso_sync_report,
    ...
    def test_display_report_no_importer_or_distributor(self, _display_iso_sync_report,
                                                       ...
                                                       _display_publish_report):
        """
        When there is not an importer or a distributor passed, none of the display functions
        should be called.
        """
        progress_report = {}
        renderer = status.ISOStatusRenderer(self.context)
    ...
    @mock.patch('pulp_rpm.extensions.admin.iso.status.ISOStatusRenderer._display_iso_sync_report',
                side_effect=status.ISOStatusRenderer._display_iso_sync_report, autospec=True)
    def test_display_report_only_distributor(self, _display_iso_sync_report,
                                             _display_manifest_sync_report,
                                             _display_publish_report):
        """
    ...
    @mock.patch('pulp_rpm.extensions.admin.iso.status.ISOStatusRenderer._display_iso_sync_report',
                side_effect=status.ISOStatusRenderer._display_iso_sync_report, autospec=True)
    def test_display_report_only_importer(self, _display_iso_sync_report,
                                          _display_manifest_sync_report,
                                          _display_publish_report):
        """
    ...
    def test__display_iso_sync_report_during_complete_stage(self):
        ...
        sync_report = progress.SyncProgressReport(
            conduit, num_isos=3, num_isos_finished=3, total_bytes=total_bytes,
    ...
    def test__display_iso_sync_report_during_iso_stage_with_isos(self):
        """
        Test the ISOStatusRenderer._display_iso_sync_report method when the SyncProgressReport
        has entered the ISO retrieval stage (with three ISOs to download) from the manifest
        retrieval stage. It should display progress to the user.
        """
        conduit = mock.MagicMock()
        finished_bytes = 12
        total_bytes = 1204
        ...
        # The _sync_state should have been updated to reflect the ISO downloading stage being in
        # progress
    ...
    def test__display_iso_sync_report_during_manifest_stage(self):
        ...
        conduit = mock.MagicMock()
        sync_report = progress.SyncProgressReport(
            conduit, state=progress.SyncProgressReport.STATE_MANIFEST_IN_PROGRESS)
        renderer = status.ISOStatusRenderer(self.context)
    ...
    def test__display_manifest_sync_report_manifest_failed(self):
        ...
        sync_report = progress.SyncProgressReport(
            conduit, error_message=error_message,
    ...
    def test__display_publish_report_failed(self):
        ...
        sync_report = progress.PublishProgressReport(
            conduit, error_message=error_message, state=progress.PublishProgressReport.STATE_FAILED)
        renderer = status.ISOStatusRenderer(self.context)
        renderer.prompt.reset_mock()

        renderer._display_publish_report(sync_report)

        # There should be two prints, one to tell the user it failed, and another to tell the
        # user the message
        self.assertEqual(len(renderer.prompt.mock_calls), 2)
        self.assertEqual(renderer.prompt.render_failure_message.mock_calls[0][2]['tag'],
                         'publish_failed')
    ...
    def test__display_publish_report_not_started(self):
        ...
        conduit = mock.MagicMock()
        sync_report = progress.PublishProgressReport(
            conduit, state=progress.PublishProgressReport.STATE_NOT_STARTED)
        renderer = status.ISOStatusRenderer(self.context)
        renderer.prompt.reset_mock()
    ...
    def test__display_publish_report_success(self):
        ...
        conduit = mock.MagicMock()
        sync_report = progress.PublishProgressReport(
            conduit, state=progress.PublishProgressReport.STATE_COMPLETE)
        renderer = status.ISOStatusRenderer(self.context)
        renderer.prompt.reset_mock()
