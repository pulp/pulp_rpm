...
class TestISOSearchCommand(PulpClientTests):
    ...
    def setUp(self):
        ...
        # Let's simulate the response from the backend for searching the ISOs. This data was
        # taken from a live
        # Pulp server
        fake_isos = mock.MagicMock()
        fake_isos.response_body = [
            {u'updated': u'2013-06-13T00:16:19Z', u'repo_id': u'cdn',
             u'created': u'2013-06-13T00:16:19Z',
             ...
             u'metadata': {
                 u'_storage_path': u'/var/lib/pulp/content/iso/SHA1SUM/'
                                   u'705406710465312e060e2668abbc3d01cce7f7ad1a0ed4cceeee212f0cb'
                                   u'e6689/960/SHA1SUM',
                 u'name': u'SHA1SUM',
            ...
            {u'updated': u'2013-06-13T00:16:20Z', u'repo_id': u'cdn',
             ...
             u'metadata': {
                 u'_storage_path': u'/var/lib/pulp/content/iso/SHA256SUM/'
                                   u'26b8fc26aa337c23592cb67e7c05fc0eff4e0d2eb5d70e74c6b27d57b'
                                   u'ecce313/984/SHA256SUM',
                 u'name': u'SHA256SUM',
    ...
    def test_search_isos_with_details(self):
        ...
        # There should be one call to the search binding. We'll need to build out the options
        # that we will
        # expect to be passed to the search binding. To do that, we'll find all possible options
        # and set them to
        # None
        ...
        # Now let's fill out the --details flag as being set True, add the ISO type ID,
        # and remove repo-id
        ...
        self.context.server.repo_unit.search.assert_called_once_with('a_repo',
    ...
    def test_search_isos_without_details(self):
        ...
        massaged_isos = [
            iso['metadata'] for iso in
            self.context.server.repo_unit.search.return_value.response_body]
