...
from pulp_smash.pulp3.utils import delete_orphans, gen_distribution, gen_repo, sync

from pulp_rpm.tests.functional.utils import gen_rpm_remote
from pulp_rpm.tests.functional.constants import RPM_DISTRIBUTION_PATH, RPM_REMOTE_PATH
...
class PackageManagerConsumeTestCase(unittest.TestCase):
    ...
    def do_test(self, policy):
        ...
        repo_path = gen_yum_config_file(
            self.cfg,
            baseurl=urljoin(self.cfg.get_content_host_base_url(), "//" + distribution["base_url"]),
            name=repo["name"],
