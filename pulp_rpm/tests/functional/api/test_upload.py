...
def test_upload_returns_client_error_when_artifact_file_exists_without_db_record(
    ...
    """RPM upload returns a 4xx error (not 500) when the artifact file exists in storage
    but the corresponding Artifact DB record is absent.

    Regression test for https://github.com/pulp/pulp_rpm/issues/4574

    The inconsistent state (file on disk, no DB record) is created by:
    1. Uploading the RPM as a bare artifact so the file lands in artifact storage.
    2. Deleting the artifact via the API, which removes both the DB record and the file.
    3. Copying the file back to its storage path, leaving the storage inconsistent.
    ...
    # Upload the RPM as a bare artifact so its file is placed in artifact storage.
    ...
    # Remove the artifact — this deletes both the DB record and the file from storage.
    pulpcore_bindings.ArtifactsApi.delete(artifact.pulp_href)
    assert not artifact_storage_path.exists()

    # Restore the file to its storage path, creating the inconsistent state.
    artifact_storage_path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(str(backup_path), str(artifact_storage_path))

    # The upload must NOT return HTTP 500.  Before the fix a ValueError propagated
    # unhandled through DRF and produced a 500 response.
