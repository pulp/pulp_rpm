    ...
    * `Pulp Smash #870 <https://github.com/PulpQE/pulp-smash/issues/870>`_
    ...
    def test_01_create_content_unit(self):
        ...
        self.content_unit.update(
            self.client.post(
                RPM_CONTENT_PATH,
                {"_artifact": self.artifact["_href"], "filename": RPM_PACKAGE_FILENAME},
            )
        )
    ...
    def test_02_read_content_units(self):
        ...
        page = self.client.get(RPM_CONTENT_PATH, params={"pkgId": self.content_unit["pkgId"]})
...
class DuplicateContentUnit(unittest.TestCase):
    ...
    *  `Pulp #4125 <https://pulp.plan.io/issue/4125>`_
    ...
    def test_raise_error(self):
        ...
        attrs = {"_artifact": artifact["_href"], "filename": RPM_PACKAGE_FILENAME}
        ...
        response = api.Client(self.cfg, api.echo_handler).post(RPM_CONTENT_PATH, attrs)
        ...
        for key in keywords:
            self.assertIn(
                key.lower(), response.json()["non_field_errors"][0].lower(), response.json()
            )
