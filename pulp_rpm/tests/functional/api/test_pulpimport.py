...
def test_create_missing_repos(
    ...
    body = {"url": RPM_UNSIGNED_FIXTURE_URL, "policy": "immediate"}
    entity_map["rpm-remote"] = rpm_rpmremote_factory(**body)
    body = {"url": RPM_KICKSTART_FIXTURE_URL, "policy": "immediate"}
    entity_map["ks-remote"] = rpm_rpmremote_factory(**body)

    # Step 2, repositories
    entity_map["rpm-repo"] = rpm_repository_factory()
    entity_map["ks-repo"] = rpm_repository_factory()

    # Step 3, sync
    repository_sync_data = RpmRepositorySyncURL(remote=entity_map["rpm-remote"].pulp_href)
    sync_response = rpm_repository_api.sync(entity_map["rpm-repo"].pulp_href, repository_sync_data)
    monitor_task(sync_response.task)

    repository_sync_data = RpmRepositorySyncURL(remote=entity_map["ks-remote"].pulp_href)
    sync_response = rpm_repository_api.sync(entity_map["ks-repo"].pulp_href, repository_sync_data)
    monitor_task(sync_response.task)
    # Read back sync'd ks-repo, so we can compare post-import
    entity_map["ks-repo"] = rpm_repository_api.read(entity_map["ks-repo"].pulp_href)
