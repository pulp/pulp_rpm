...
class PackageSerializer(SingleArtifactContentSerializer):
    ...
    epoch = serializers.CharField(
        help_text=_("The package's epoch"), allow_blank=True, required=False
    )
    version = serializers.CharField(help_text=_("The version of the package. For example, '2.8.0'"))
    ...
    checksum_type = serializers.CharField(help_text=_("Type of checksum, e.g. 'sha256', 'md5'"))

    summary = serializers.CharField(
        help_text=_("Short description of the packaged software"), allow_blank=True, required=False
    )
    ...
    changelogs = serializers.CharField(
        help_text=_("Changelogs that package contains"), default="[]", required=False
    )
    files = serializers.CharField(
        help_text=_("Files that package contains"), default="[]", required=False
    )

    requires = serializers.CharField(
        help_text=_("Capabilities the package requires"), default="[]", required=False
    )
    provides = serializers.CharField(
        help_text=_("Capabilities the package provides"), default="[]", required=False
    )
    conflicts = serializers.CharField(
        help_text=_("Capabilities the package conflicts"), default="[]", required=False
    )
    obsoletes = serializers.CharField(
        help_text=_("Capabilities the package obsoletes"), default="[]", required=False
    )
    suggests = serializers.CharField(
        help_text=_("Capabilities the package suggests"), default="[]", required=False
    )
    enhances = serializers.CharField(
        help_text=_("Capabilities the package enhances"), default="[]", required=False
    )
    recommends = serializers.CharField(
        help_text=_("Capabilities the package recommends"), default="[]", required=False
    )
    supplements = serializers.CharField(
        help_text=_("Capabilities the package supplements"), default="[]", required=False
    )

    location_base = serializers.CharField(
        help_text=_("Base location of this package"), allow_blank=True, required=False
    )
    ...
    rpm_header_start = serializers.IntegerField(help_text=_("First byte of the header"))
    rpm_header_end = serializers.IntegerField(help_text=_("Last byte of the header"))
    ...
    size_package = serializers.IntegerField(help_text=_("Size, in bytes, of the package"))
...
class UpdateRecordSerializer(NoArtifactContentSerializer):
    ...
    id = serializers.CharField(help_text=_("Update id (short update name, e.g. RHEA-2013:1777)"))
    ...
    fromstr = serializers.CharField(help_text=_("Source of the update (e.g. security@redhat.com)"))
    status = serializers.CharField(help_text=_("Update status ('final', ...)"))
    ...
    pushcount = serializers.CharField(help_text=_("Push count"))

    pkglist = UpdateCollectionField(source="pk", read_only=True, help_text=_("List of packages"))
...
class MinimalUpdateRecordSerializer(UpdateRecordSerializer):
    ...
    class Meta:
        fields = NoArtifactContentSerializer.Meta.fields + ("id", "title", "severity", "type")
        model = UpdateRecord
...
class OneShotUploadSerializer(serializers.Serializer):
    ...
    file = serializers.FileField(help_text=_("The rpm file."), required=True)
...
class CopySerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if source_repo and source_repo_version:
            raise serializers.ValidationError(
                _(
                    "Either the 'source_repo' or 'source_repo_version' need to be specified "
                    "but not both."
                )
            )
                    ...
                    raise serializers.ValidationError(
                        _(
                            "'{type}' is an invalid type, please use one of {choices}".format(
                                type=t, choices=list(RPM_PLUGIN_TYPE_CHOICE_MAP.keys())
                            )
                        )
                    )
