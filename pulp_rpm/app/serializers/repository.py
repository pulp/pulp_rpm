    ...
    def validate(self, data):
        ...
        if checksum_type := data.get("checksum_type"):
            ...
            if checksum_type not in ALLOWED_PUBLISH_CHECKSUMS:
                raise serializers.ValidationError({"checksum_type": _(ALLOWED_PUBLISH_CHECKSUM_ERROR_MSG)})
...
class RpmPublicationSerializer(PublicationSerializer):
    ...
    def validate(self, data):
        ...
        if checksum_type := data.get("checksum_type"):
            ...
            if checksum_type not in ALLOWED_PUBLISH_CHECKSUMS:
                raise serializers.ValidationError({field: _(ALLOWED_PUBLISH_CHECKSUM_ERROR_MSG)})
