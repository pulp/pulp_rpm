...
cd /home/travis/build/pulp/pulp_rpm/
export REPORTED_VERSION=$(http :24817/pulp/api/v3/status/ | jq --arg plugin pulp_rpm -r '.versions[] | select(.component == $plugin) | .version')
